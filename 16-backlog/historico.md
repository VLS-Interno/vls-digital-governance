...
| GOV-5 | Decidir aposentadoria de `BACKLOG-VLS` consolidando em `vls-digital-governance/16-backlog/` | Achado 25/08 | Decisão do usuário 01/09: arquivar agora (conteúdo já consolidado via GOV-1/GOV-2 + Fases 5-6) | Settings → Archive, via navegador autenticado | 01/09/2026 | MÉTRICA: confirmado ao vivo — "This repository was archived by the owner on Sep 1, 2026. It is now read-only." | — | — |
| GOV-6 *(corrigido 01/09 — auditoria de fechamento achou que esta linha nunca tinha sido adicionada aqui, apesar de já executada)* | Marcar `CRM-VLS/docs/referencia_backend_supabase.md` (stale 2 meses, autodeclarado "fonte da verdade") como histórico | Achado 25/08 | — | Commit `2704417` no `CRM-VLS` (junto com os outros 3 docs stale) | 01/09/2026 | CÓDIGO: banner de aviso confirmado no topo do arquivo | — | — |
