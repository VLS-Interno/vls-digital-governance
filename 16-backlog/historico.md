...
**Fila de investigação/validação — não é histórico, não é futuro, não é backlog ativo.** 11 itens seguem `NAO_CONFIRMADO`, cada um com pendência específica registrada: `GOV-3` (relação `VLS-Constutora-TI`×`VLS-Interno`, decisão prática já tomada — manter em `VLS-Interno` — mas fato técnico não confirmado), `GOV-4` (promover Baseline 25/08 a Nível 1), `GOV-5` (aposentar `BACKLOG-VLS` de fato), `M1` (status Meta do teste 80km), `Spend cap "Ricardo Valese"` (P0, aguarda MCP com acesso à conta), `CAT-5` (família `followup_corretor_*` abandonada?), `E1` (Google Calendar idempotência), `TF-6 2º nó` (dual-write draft, motivo da edição de 28/08 desconhecido), `G1` (identificar os 2 gaps P2), `X3` (Instagram "Robson" hardcoded), `R4` (residuais Sheets Córdoba). Permanecem na Seção 8 do Backlog Executivo até confirmação.
